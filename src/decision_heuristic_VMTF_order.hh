...
class DecisionHeuristicVMTForder: public DecisionHeuristic {
...
inline void DecisionHeuristicVMTForder::notifyStart() {
...
inline void DecisionHeuristicVMTForder::notifyAssigned(Literal l) {
  saved_phase[var(l) - 1] = sign(l);
}

inline void DecisionHeuristicVMTForder::notifyEligible(Variable v) {
  if (decision_list[v - 1].timestamp > decision_list[next_search - 1].timestamp && !is_auxiliary[v - 1]) {
    overflow_queue.push(v);
  }
}

inline void DecisionHeuristicVMTForder::clearOverflowQueue() {
...
inline uint32_t DecisionHeuristicVMTForder::maxTimestampEligible() {
